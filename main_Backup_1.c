...
long graph(uint16_t value)
{
	...
	if(posx==0)//adjust to middle of screen at initial position
	{
		while((corr-value)!=120)corr++;
		posy = corr-value;//calculate correction
		ili9341_drawLine(0, 120, 1, posy, GREENYELLOW);
	}else
	{
		posy = corr-value;
		ili9341_drawLine(posx-1, posy_old, posx, posy, YELLOW);
		ili9341_drawLine(posx-1, posy_old+1, posx, posy+1, YELLOW);
	}
